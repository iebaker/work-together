...
public class Node {
	 ...
	 */
	private java.util.Map<String, Node> m_children = new HashMap<String, Node>();

	/**
	 * A list of the elements which are owned by this Node
	 */
	private java.util.List<SliceElement> m_elements = new ArrayList<SliceElement>();

	/**
	 * The padding values of this node
	 */
	private float[] m_padding = new float[]{0, 0, 0, 0};

	/**
	 * The Gravity value of this node (which side BLOCKS style children will fall to)
	 */
	private Slice.G m_gravity = Slice.G.NONE;

	/**
	 * The parent node of this node
	 */
	private Node m_parent_node = null;

	/**
	 * The containing screen of this node
	 */
	protected Screen m_parent_screen = null;
	...
	public void setPadding(float left, float top, float right, float bottom) {
		m_padding = new float[]{left, top, right, bottom};
	}
	...
	public void clearChildren() {
		m_children = new HashMap<String, Node>();
	}
	...
	public void attach(SliceElement... elems) {
		for(SliceElement se : elems) {
			se.setAttrLocation(new Vec2f(this.attrLocation.x + m_padding[0], this.attrLocation.y + m_padding[1]));
	...
	private float computePadding(Slice.D d, Float[] sizes, float minP) {
		...
		float useful_height = attrSize.y - m_padding[1] - m_padding[3];
		float useful_width = attrSize.x - m_padding[0] - m_padding[2];
		float leftover = 0;
	...
	private java.util.List<Node> verticalDivide(Float[] splits, String[] names, float p) {
		...
		float current_y = attrLocation.y + m_padding[1];
		float current_x = attrLocation.x + m_padding[0];

		//The actual space available to use
		float useful_height = attrSize.y - m_padding[1] - m_padding[3] - (splits.length * p);
		float useful_width = attrSize.x - m_padding[0] - m_padding[2]; 

		//Loop through, creating and adding child nodes
		for(int index = 0; index < names.length - 1; ++index) {
			Node temp = new Node(names[index], m_parent_screen, this, new Vec2f(current_x, current_y), new Vec2f(useful_width, useful_height * splits[index]));
			return_value.add(temp);
			m_children.put(names[index], temp);
			current_y += (useful_height * splits[index] + p);
			leftover -= splits[index];
		}

		//Create and add the last node
		Node temp = new Node(names[names.length - 1], m_parent_screen, this, new Vec2f(current_x, current_y), new Vec2f(useful_width, useful_height * leftover));
		return_value.add(temp);
		m_children.put(names[names.length - 1], temp);
	...
	private java.util.List<Node> horizontalDivide(Float[] splits, String[] names, float p) {
		...
		float current_x = attrLocation.x + m_padding[0];
		float useful_height = attrSize.y - m_padding[1] - m_padding[3];
		float useful_width = attrSize.x - m_padding[0] - m_padding[2] - (splits.length * p);

		for(int index = 0; index < names.length - 1; ++index) {
			Node temp = new Node(names[index], m_parent_screen, this, new Vec2f(current_x, current_y), new Vec2f(useful_width * splits[index], useful_height));
			return_value.add(temp);
			m_children.put(names[index], temp);
		...
		Node temp = new Node(names[names.length - 1], m_parent_screen, this, new Vec2f(current_x, current_y), new Vec2f(useful_width * leftover, useful_height));
		return_value.add(temp);
		m_children.put(names[names.length - 1], temp);
	...
	private java.util.List<Node> verticalFloating(boolean justify, Float[] sizes, String[] names, float p) {
		...
		float current_x = attrLocation.x + m_padding[1];
		float current_y;
		float useful_height = attrSize.y - m_padding[1] - m_padding[3];
		float useful_width = attrSize.x - m_padding[0] - m_padding[2];

		if(justify) {
			if(m_gravity == Slice.G.SOUTH) {
				current_y = attrLocation.y + attrSize.y - m_padding[3];
				for(int index = sizes.length - 1; index >= 0; --index) {
					Node temp = new Node(names[index], m_parent_screen, this, new Vec2f(current_x, current_y - sizes[index]), new Vec2f(useful_width, sizes[index]));
					return_value.add(temp);
					m_children.put(names[index], temp);
					current_y -= (sizes[index] + p);
				}
			} else {
				current_y = attrLocation.y + m_padding[1];
				for(int index = 0; index < sizes.length; ++index) {
					Node temp = new Node(names[index], m_parent_screen, this, new Vec2f(current_x, current_y), new Vec2f(useful_width, sizes[index]));
					return_value.add(temp);
					m_children.put(names[index], temp);
					current_y += (sizes[index] + p);
				}
			...
			current_y = attrLocation.y + m_padding[1] + p;
			for(int index = 0; index < sizes.length; ++index) {
				Node temp = new Node(names[index], m_parent_screen, this, new Vec2f(current_x, current_y), new Vec2f(useful_width, sizes[index]));
				return_value.add(temp);
				m_children.put(names[index], temp);
	...
	private java.util.List<Node> horizontalFloating(boolean justify, Float[] sizes, String[] names, float p) {
		...
		float current_x;
		float current_y = attrLocation.y + m_padding[0];
		float useful_height = attrSize.y - m_padding[1] - m_padding[3];
		float useful_width = attrSize.x - m_padding[0] - m_padding[2];

		if(justify) {
			if(m_gravity == Slice.G.EAST) {
				current_x = attrLocation.x + attrSize.x - m_padding[2];
				for(int index = sizes.length - 1; index >= 0; --index) {
					Node temp = new Node(names[index], m_parent_screen, this, new Vec2f(current_x - sizes[index], current_y), new Vec2f(sizes[index], useful_height));
					return_value.add(temp);
					m_children.put(names[index], temp);
					current_x -= (sizes[index] + p);
				}
			} else {
				current_x = attrLocation.x + m_padding[0];
				for(int index = 0; index < sizes.length; ++index) {
					Node temp = new Node(names[index], m_parent_screen, this, new Vec2f(current_x, current_y), new Vec2f(sizes[index], useful_height));
					return_value.add(temp);
					m_children.put(names[index], temp);
					current_x -= (sizes[index] + p);
				}
			...
			current_x = attrLocation.x + m_padding[0] + p;
			for(int index = 0; index < sizes.length; ++index) {
				Node temp = new Node(names[index], m_parent_screen, this, new Vec2f(current_x, current_y), new Vec2f(sizes[index], useful_height));
				return_value.add(temp);
				m_children.put(names[index], temp);
